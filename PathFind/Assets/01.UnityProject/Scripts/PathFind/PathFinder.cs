    ...
    private IEnumerator DelayFindPath_Astar(float delay_)
    {
        ...
        while(isFoundDestination == false && isNowayToGo == false)
        {
            ...
            else
            {
                ...
                List<int> nextSearchIdx1Ds = mapBoard.GetTileIdx2D_Around4ways(
                    minCostNode.Terrain.TileIdx2D);
                ...
                foreach(var nextIdx1D in nextSearchIdx1Ds)
                {
                    ...
                    GFunc.Log($"nextNode : {nextNode}, nextNode.Terrain : {nextNode.Terrain}");
